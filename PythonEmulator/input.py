from .EmulatorCore import Ram, SerialBase
class Keyboard():
  ...
  def getSerialWrapper(self):
    return SerialKeyboardWrapper(self.buffer, self.mmioHandle)
class SerialKeyboardWrapper(SerialBase):
  def mmioHandle(self):
    if self.data[0] == 0:
      self.mmio()
      self.data[0] = self.objectRam[self.pointer]
      if self.data[0] != 0:
        self.pointer += 1 
